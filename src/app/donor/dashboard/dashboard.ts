...
export class DonorDashboard implements OnInit {
  ...
  ngOnInit() {
    ...
    this.user.donationStatus = this.user.donationStatus || null;
  ...
  checkDonationEligibility() {
    ...
    if (diffDays >= 90) {
      ...
      this.daysLeft = Math.ceil(90 - diffDays);
    }
  ...
  donateBlood() {
    if (!this.canDonate) {
      alert(`You can donate after ${this.daysLeft} days.`);
      return;
    }

    if (this.user.donationStatus === 'PENDING') {
      alert('You already have a pending donation request.');
      return;
    }

    const confirmDonate = confirm(
      'Are you sure you want to request to donate blood?'
    );
    ...
    this.userService.updateUser(this.user).subscribe({
      next: () => {
        ...
        alert('Donation request sent successfully! Once approved, you can download your certificate.');
      },
      error: (err) => {
        console.error('Failed to send donation request', err);
        alert('Failed to send donation request. Please try again.');
        this.user.donationStatus = null;
      }
    });
  }
